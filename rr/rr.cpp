...
SOCKET ConnectHost(DWORD dwIP, WORD wPort)//连接指定IP和端口
{
	...
	ioctlsocket(sockid,FIONBIO, &dwArg);
	...
	{
		printf("[errno:%d]",WSAGetLastError());
		goto error;
	}
	return sockid;
error:
...
DWORD WINAPI PortTransfer_1(LPVOID lParam)
{
	...
	printf("\r\nThreadID: %d ==> ConnectHost(%s:%d)...", \
		nTimes, ConfigInfo->GlobalData.szIP, ConfigInfo->GlobalData.wPort);
	//先连接到目标计算机的服务
	unsigned long dwArg = 1;
	ioctlsocket(ClientSock,FIONBIO, &dwArg);
